...
import type { ExcalidrawHexagonElement } from "./types";
...
export const getHexagonPoints = (
  ...
  const R = Math.min(height / 2, width / Math.sqrt(3));

  const R_cos30 = (R * Math.sqrt(3)) / 2;
  const R_sin30 = R / 2;

  return [
    [cx, cy - R],
    [cx + R_cos30, cy - R_sin30],
    [cx + R_cos30, cy + R_sin30],
    [cx, cy + R],
    [cx - R_cos30, cy + R_sin30],
    [cx - R_cos30, cy - R_sin30],
